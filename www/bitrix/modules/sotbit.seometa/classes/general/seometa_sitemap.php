...
class CSeoMetaSitemap extends CCSeoMeta
{
    ...
    {                                          
        ...
        if ($Condition['DATA']['All'] == 'AND' && $Condition['DATA']['True']) // AND
        {
            ...
            {                     
                ...
                {                                
                    ...
                    if (isset( $Prop['ALL'] ) && is_array( $Prop['ALL'] )) // if not value
                    {
                        if ($PagesCnt > 0)
                        {
                            ...
                            {                        
                                ...
                                {                        
                                    ...
                                    $Pages[$Cnt][$PropID]['VALUE']=$Prop['VALUE']; 
                                }
                            }
                        }
                        else
                        {                                                 
                            ...
                            {                                 
                                ...
                                $Pages[$k][$PropID]['VALUE'] = $Prop['VALUE'];   
                                ++$k;                           
                            }
                        }
                    }
                    ...
                    elseif ($PropID == "FILTER")
                    {
                        if ($PagesCnt > 0)
                        {
                            foreach ( $Pages as &$Page )
                            {
                                $Page['FILTER'][key( $Prop )]['TYPE'][] = $Prop[key( $Prop )]['TYPE'][0];
                                $Page['FILTER'][key( $Prop )]['ID'][] = $Prop[key( $Prop )]['ID'][0];
                                $Page['FILTER'][key( $Prop )]['VALUE'][] = $Prop[key( $Prop )]['VALUE'][0];
                            }
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            $Pages[0][$PropID]['VALUE'] = $Prop['VALUE'];       
                        }
                    }
                }     
            }                                 
        ...
        {   
            if(self::$isGenerateChpu){              
                foreach($result as $k =>$arrayCond){        
                    foreach($arrayCond as $kk => $CondPages){
                        if(isset($CondPages['ALL'])){     
                            foreach($CondPages['ALL'] as $id => $cond){  
                                foreach($cond as $index => $val){
                                    $i=0;   
                                    foreach($val as $p => $str){  
                                        $arr[$i][$kk]['CODE'] = $CondPages['CODE'];
                                        $arr[$i][$kk]['VALUE'] = $CondPages['VALUE'];
                                        foreach($str as $q=>$qq){                       
                                            $arr[$i][$kk][$id][$index][] = $qq;
                                        }                                    
                                        $i++;   
                                    }              
                                }                                     
    ...
    public function PrepareConditions($conditions, $ConditionSections)
    {
        ...
        {                                     
            ...
            } elseif(strpos( $condition['CLASS_ID'], 'FilterProperty' ) !== false) // If filter property
            {                     
                $arCond = explode( 'FilterProperty', str_replace( 'CondIB', '', $condition['CLASS_ID'] ) );
                
                    $arCondProp = explode( ':', $arCond[1] );   
                    $IdIblock = $arCondProp[0];
                    $IdProperty = $arCondProp[1];
                          
                $PropVals[$i] = self::GetPropVal( $condition['DATA']['value'], $IdProperty, $IdIblock, $condition['DATA']['logic'], $ConditionSections, $arCond[0] );
            } else
            {
    ...
    public function SetListOfProps($FilterType)
    {
        ...
        foreach ( self::$AllPropsForSortByIblock as $IdIblock => $Props )
        {
            ...
            elseif($IdIblock == "PRICES")
            {
                $TmpAllProps['PRICES'] = $Props;
            }
            elseif($IdIblock == "FILTER")
            {
                $TmpAllProps['FILTER'] = $Props;
            }
        }
        self::$AllPropsForSort = array();
        if ($FilterType == "BITRIX" || strtoupper($FilterType)=='BITRIX_CHPU' || strtoupper($FilterType)=='BITRIX_NOT_CHPU')
        {
            if (isset( $TmpAllProps['PRICES'] ))
                self::$AllPropsForSort[]["ID"] = "PRICES";
            if (isset( $TmpAllProps['FILTER'] ))
                self::$AllPropsForSort[]["ID"] = "FILTER";
        }
        ...
        {            
            foreach ( $Pages as $i => $Page ){   
                if (isset( $Page[$Prop['ID']] )){    
                    ...
                    //$return[$i][$Prop['ID']]['VALUE'] = $Page['VALUE'];
                }
            }
        }
        return $return;
    ...
    private function GetValueOfTextProp($ConditionValue, $IdIblock, $IdProperty, $ConditionSections, $PropertyCode)
    {
        ...
        else
        {
            ...
            $return[$IdProperty]['BITRIX'][1][] = urlencode( strtolower($ConditionValue) );
        }
        $return[$IdProperty]['CODE']=$PropertyCode;
        if(!isset($return[$IdProperty]['VALUE']))
            $return[$IdProperty]['VALUE']=$ConditionValue;
