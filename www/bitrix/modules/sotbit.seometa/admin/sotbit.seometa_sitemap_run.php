...
if (isset( $arSitemap['SETTINGS']['FILTER_TYPE'] ) && !is_null( $arSitemap['SETTINGS']['FILTER_TYPE'] ))
{
    ...
    elseif ($FilterType == 'COMBOX' && $FilterCHPU)
    {
...
if (file_exists( $arSite['ABS_DOC_ROOT'] . $arSite['DIR'] . $arSitemap['SETTINGS']['FILENAME_INDEX'] ))
{
    ...
    {                           
        ...
        CSeoMetaSitemap::SetListOfProps( ($FilterType=='COMBOX')?'MISSSHOP':$FilterType );
            ...
            foreach ( $CondVals['PAGES'] as $Page )
            {
                    ...
                    {                   
                        ...
                        {                                          
                            foreach ( $CondValProps as $PriceCode => $PriceProps )
                            {
                                ...
                                $cond_properties['FILTER'][$PriceCode]['FROM'] = $ValMin;
                                $cond_properties['FILTER'][$PriceCode]['TO'] = $ValMax;     
                ...
                foreach($cond_properties as $code => $vals){    
                    ...
                    } elseif ($code == 'FILTER')
                    {
                        ...
                        if(intval($code))
                            $pr = \CIBlockProperty::GetList(array(), array( 'ID'=>$code , 'IBLOCK_ID' => $CondVals['INFOBLOCK'] ))->fetch();
                        else
                            $pr = \CIBlockProperty::GetList(array(), array( 'CODE'=>$code , 'IBLOCK_ID' => $CondVals['INFOBLOCK'] ))->fetch();
                        ...
                        } elseif($pr['PROPERTY_TYPE']=='L' ) {

                            $property_enums = \CIBlockPropertyEnum::GetList(
                                Array("DEF"=>"DESC", "SORT"=>"ASC"),
                                Array("IBLOCK_ID"=>$CondVals['INFOBLOCK'] , "PROPERTY_ID"=> $pr["ID"])
                            );

                            while($enumField = $property_enums->GetNext())
                            {
                                if($enumField["XML_ID"]==$prop_string_values[$pr['ID']] || $enumField["ID"]==$prop_string_values[$pr['ID']] ) {
                                    $arFilter['PROPERTY_' . $pr['ID'] . '_VALUE'][] = $enumField["VALUE"];

                                }
                            }
                        } elseif( $pr['PROPERTY_TYPE']=='E') {
                            $arFilter['PROPERTY_'.$pr['ID'].'_VALUE'] = $prop_string_values[$pr['ID']]?$prop_string_values[$pr['ID']]:$vals;
