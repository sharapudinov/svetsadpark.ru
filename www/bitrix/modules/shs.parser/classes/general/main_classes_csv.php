...
class SotbitCsvParser extends SotbitXmlParser{
    ...
    {      
        ...
        //file_put_contents(dirname(__FILE__)."/log.scv", $csvFile->__buffer);
    ...
    {                                                           
       ...
       /* $str = mb_detect_encoding($this->arFields["NAME"], "auto");
        file_put_contents(dirname(__FILE__)."/log.log", $str."\n", FILE_APPEND); */
   ...
   protected function parseCatalogParrentSectionCsv(&$el)             //parse parent section
   {
            ...
            if(empty($parent_name)) return false; 
                ...
                if(!empty($parent_id)){
                    ...
                    if(count($this->settings["catalog"]["id_category_main"]) > 0){                                                                                                                                                     
                        $parent_id = $this->issetMainSectionCatalogByNameÑsv($parent_name, (isset($parent_section_name)&& !empty($parent_section_name))?$parent_section_name:''); 
                    }                                                 
                    if(!$parent_id){
                ...
                }/* elseif(count($this->settings["catalog"]["id_category_main"]) > 0) {                                                                                             
                }  */
            }               
        } else {
    ...
    protected function issetMainSectionCatalogByNameÑsv($parent_name = '', $section_name = '')
    {
        if(empty($parent_name)) return false;
