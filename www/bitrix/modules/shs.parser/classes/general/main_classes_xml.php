...
class SotbitXmlParser extends SotbitContentParser {
            ...
            {   
                ...
                { 
                    ...
                    {     
                        ...
                        {                            
                            //write header
                            if(!$this->WriteNewXmlFile($this->XmlHeader, $url, $id, "HEADER"))
                                continue 1; 
    ...
    protected function GetArrSectionXml()
    {
        ...
        foreach ($arr_section as $el_section)
        {
            ...
            if(!empty($this->settings['catalog']["description_category"]))
            {
                ...
                elseif(empty($arr_description['attr']))
                {
                    $arr[$section_id]['description'] = strip_tags(trim(pq($el_section)->find($arr_description["path"])->html()));
                }
            }
    ...
    protected function parseUserFields($key, $value){   
        $return = '';
        $res = CUserTypeEntity::GetList(array(), array('ENTITY_ID' => 'IBLOCK_'.$this->iblock_id.'_SECTION','FIELD_NAME'=>$key))->fetch();     
        if(!empty($res)){
            switch($res['USER_TYPE_ID']){
                ...
                case 'boolean':
                    ...
                    break;                              
                case 'enumeration':  
                    $resE = CUserFieldEnum::GetList(array(),array('USER_FIELD_ID'=>$res['ID'], 'VALUE'=>$value))->fetch();                                                                     
                    if(!empty($resE)){
                        $return = $resE['ID'];    
                    } else {
                        $arFields = array(
                            ...
                            'DEF' => 'N',
                            'SORT' => 100,
                            'XML_ID' => CUtil::translit($value,'ru',array("replace_space"=>"-","replace_other"=>"-")),
                        );
                        global $DB;
                        $DB->Add("b_user_field_enum", $arFields);        
                        $resE = CUserFieldEnum::GetList(array(),array('USER_FIELD_ID'=>$key, 'VALUE'=>$value))->fetch();
                        if($resE){
                            $return = $resE['ID'];   
                        }
                    }
         ...
         if(is_array($cats) and  isset($cats[$parent_id]))
         {
            foreach($cats[$parent_id] as $id => $cat)
            {
               ...
               {   
                   ...
                   $this->addSectionXlm(array("id_section" => $id, "text_section" => $cat["text"], "id_parrent" => $parent_sec,'description'=>$cat['description'],'description_type'=>$cat['description_type'],'pic'=>$cat['pic'],'user_fields'=>$cat['user_fields'],));
               }
               else
               {
                   if($cat["parentId"] == 0)
                   ...
                   $this->UpdateSectionXml($id_sec, array("id_section" => $id, "text_section" => $cat["text"], "id_parrent" => $parent_sec,'description'=>$cat['description'],'description_type'=>$cat['description_type'],'pic'=>$cat['pic'],'user_fields'=>$cat['user_fields'],));
               }
