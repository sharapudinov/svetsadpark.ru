...
class SotbitContentParser {
    ...
    {                
        ...
        $this->tmp = $shs_TMP;
        $this->settings = $this->sotbitParserDecoderSettings(unserialize(base64_decode($shs_SETTINGS)));        //fix by Zaicev
        $this->header_url = "";
        $this->sleep = (int)$this->settings[$this->typeN]["sleep"];                                                                                             
        $pr = is_array($this->settings["proxy"]['servers'])?$this->settings["proxy"]['servers']:array(); 
        if($this->settings[$this->typeN]["proxy"]!='' && $this->settings["proxy"]['username_password']!='')
            $proxy = array('ip'=>$this->settings[$this->typeN]["proxy"], 'username_password'=>$this->settings["proxy"]['username_password']);
        else
            $proxy = array();
    ...
    public function startParser($agent=false){                                        
        ...
        if(file_exists(dirname(__FILE__)."/include/startAgent".$this->id.".txt") && !$agent)
            unlink(dirname(__FILE__)."/include/startAgent".$this->id.".txt",'start parser '.$this->id);
        ...
        if ($this->typeN=="xml")
        {
            ...
            // В переменную $this->arSortUpdate записывает поля по которым сортировать;
            // Обнуляем переменную $this->arEmptyUpdate            
            $this->GetSortFields(); 
            
            // Записывает в переменную $this->arrayIblock массив полей информационного блока. Метод статический.
            $this->getArrayIblock(); 
            $this->CheckFields($this->settings["catalog"]); //�������� ����������� ������ � ����� ������
    ...
    protected function isUpdateElement()
    {
        ...
        /*
         * $this->settings["catalog"]["update"]["active"] такой опции нет.
         * судя по всему это чекбокс "обновлять товары" на закладке "обновление\уникальность"
         * но изменение этой опции меняет состояние $this->settings['updateActive']
         * */
        //if($this->settings['updateActive'])
        if($this->settings["catalog"]["update"]["active"])                
        {
            //зачем удалять. для других элементов не понадобиться опция?
    ...
    protected function isCatalogXlsCatalog()
    {
        ...
        {
            if(
            ((isset($this->settings["catalog"]["preview_price"]) && $this->settings["catalog"]["preview_price"]!=='') || (isset($this->settings["catalog"]["detail_price"]) && $this->settings["catalog"]["detail_price"]!=='') || (isset($this->settings["catalog"]["detail_count"]) && $this->settings["catalog"]["detail_count"]!=="") || (isset($this->settings["catalog"]["preview_count"]) && $this->settings["catalog"]["preview_count"]!=="") || (isset($this->settings["catalog"]["count_default"]) && $this->settings["catalog"]["count_default"]!==""))
            ||((isset($this->catalog_detail_settings['settings']["catalog"]["detail_price"]) && $this->catalog_detail_settings['settings']["catalog"]["detail_price"]!=='') || (isset($this->catalog_detail_settings['settings']["catalog"]["detail_count"]) && $this->catalog_detail_settings['settings']["catalog"]["detail_count"]!==""))
            )
            {
                $this->isCatalog = true;
            }else $this->isCatalog = false;
    ...
    {
        /**
         * при повторном парсе деактивирует товары даже если они были в предыдущем парсе.
         * $this->settings["catalog"]["uniq"]["action"]=="A" - деактивировать товары которые были в предыдущем парсе но отсутствуют в текущем
         * данный функциона реализован в функции checkActionAgent
         * */
        /*
    ...
    protected function AddMeasureCatalogOffer()
    {
        ...
        if(!CheckVersion("14.0.0", $info->MODULE_VERSION))
        {
            if($this->settings["catalog"]["koef"]>0)
            {
                ...
                unset($arMes);
                unset($db_CAT_MEASURE_RATIO);
                unset($CAT_MEASURE_RATIO_ID);
                unset($str_CAT_MEASURE_RATIO);
            }
        }
    }
    ...
    protected function parseCatalogSelectorProperties(&$el)
    {
        ...
        foreach($arProperties as $code=>$val)
        {
            ...
            if($arProp["PROPERTY_TYPE"]=="F")
            {
                ...
                if($attr && strpos($attr,'=')!=0) {
                    $text = pq($el)->find(htmlspecialchars_decode($this->settings["catalog"]["selector_prop"][$code]))->html();
                }elseif ($attr){
    ...
    public function parseCatalogPreviewPicturePreview(&$el) //27.10.2015
    {
        ...
        {   
            ...
            $src = $this->parseCaralogFilterSrc($src);
            $src = $this->getCatalogLink($src);
            foreach(GetModuleEvents("shs.parser", "ParserPreviewPicture", true) as $arEvent) //05.06.2017
                    ExecuteModuleEventEx($arEvent, array(&$this, &$src));
    ...
    protected function parseCatalogSaveImgServer($img, $src){
        ...
        //file_put_contents(dirname(__FILE__)."/ll.log", $src."\n", FILE_APPEND);
    ...
    protected function connectCatalogPage($page)
    {
        ...
        if(!$this->page)
        {  //file_put_contents(dirname(__FILE__).'/return444444.txt',";l;l;l;"); 
    ...
    */
    public function updateProductStore($productId, $storeId, $count){
        ...
        if (is_array($arInfo)) { 
            $rsOffers = CIBlockElement::GetList(array(),array('IBLOCK_ID' => $arInfo['IBLOCK_ID'], 'PROPERTY_'.$arInfo['SKU_PROPERTY_ID'] => $productId)); 
