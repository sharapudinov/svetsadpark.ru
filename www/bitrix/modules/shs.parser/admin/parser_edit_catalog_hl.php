...
?>
   ...
   <?php /* <tr>
    ...
    <?php /* if(isset($shs_SETTINGS["catalog"]["rss_dop"]) && !empty($shs_SETTINGS["catalog"]["rss_dop"])):?>
        <?$count_url = 0;?>
...
?>
    ...
    if(isset($shs_SETTINGS["catalog"]["selector_prop"]) && !empty($shs_SETTINGS["catalog"]["selector_prop"])){
        foreach($shs_SETTINGS["catalog"]["selector_prop"] as $id => $props){
        ?>
        <tr class="dop_props_id_<?php echo $id;?>">
            <td><?php echo GetMessage('parser_field').' '.$id.':';?></td>
            <td>
                <input type="text" name="SETTINGS[catalog][selector_prop][<?php echo $id;?>]" value="<?echo $props;?>" size="40" maxlength="250">&nbsp;<a href="#" class="prev_prop_delete" data-prop-id="<?php echo $id;?>"><?php echo GetMessage('parser_delete');?></a>
            </td>
        </tr>
        <?php    
        }
    ...
    <tr>
        <td colspan="2" align="center">
            <?=SelectBoxFromArray('arrPropDop', $arProps, "", GetMessage("shs_parser_select_prop"), "");?>
            <input type="submit" id="loadDopPropHL" name="refresh" value="<?=GetMessage("shs_parser_select_prop_but")?>">
        </td>
    </tr>
