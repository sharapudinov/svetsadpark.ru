    ...
    public function sotbitParserSetSettings(&$SETTINGS)
    {
        foreach($SETTINGS as &$v)
        {
            ...
            else
            {
                ...
                {
                    $v = htmlentities(htmlspecialcharsBack($v));
                }
            } 
        }
    }
...
Class CShsParser
{
    static function startAgent($ID){
        ...
        $ID=0;                                                                   
        if(!file_exists(dirname(__FILE__)."/include/startAgent".$ID.".txt"))
            file_put_contents(dirname(__FILE__)."/include/startAgent".$ID.".txt",'start parser '.$ID);
        $rssParser = new RssContentParser();  
        $rssParser->startParser(1);      
        if(file_exists(dirname(__FILE__)."/include/startAgent".$ID.".txt"))
            unlink(dirname(__FILE__)."/include/startAgent".$ID.".txt");
