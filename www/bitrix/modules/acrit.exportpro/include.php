...
class CAcritExportproElement{
    ...
    public function ProcessBasicCsv( $dbElements, $fileExport, $page, $navPageCount, $bXls = false ){
            ...
            if( $arRowToCsv ){
                if( empty( $arPaternFields ) && ( $page == 1 ) ){
                    foreach( $arRowToCsv as $colIndex => $colValue ){
                        if( ( $this->profile["TYPE"] == "advantshop" ) && ( $colIndex == "sku_size_color_price_purchaseprice_amount" ) ){
        ...
        if( $page == 1 ){
            ...
            if( $bXls ){
                CAcritExportproExport::Save( CExportproProfile::GetXlsHeader() );
            }
        }
        ...
        if( !$bXls ){
            ...
            foreach( $arResFields as $arTuple ){
                $xlsContent .= "<tr>";
                foreach( $arTuple as $arTupleItem ){
                   $xlsContent .= "<td>".$arTupleItem."</td>";
                }
                $xlsContent .= "</tr>";
            }

            $xlsContent = ( $page == $navPageCount ) ? $xlsContent.CExportproProfile::GetXlsFooter() : $xlsContent;
            CAcritExportproExport::Save( $xlsContent );
        }
    }
    ...
    private function GetElementProperties( $arElement ){
        ...
        if( $this->catalogIncluded ){
            ...
            if( !isset( $arItem["CATALOG_PRICE_{$this->basePriceId}"] ) && ( $this->profile["USE_AUTOPRICE"] == "Y" ) ){
                if( $arMinimalPrice = CCatalogProduct::GetOptimalPrice( $arItem["ID"], 1, array( 2 ), "N", array(), SITE_ID, array() ) ){
